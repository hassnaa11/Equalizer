...
def detect_arrhythmias(ecg_signal):
    ...
    af_values = fft_values[(freqs > 50) & (freqs <= 100)]
    mi_values = fft_values[(freqs >= 0) & (freqs <= 4)]
    sr_values = fft_values[(freqs >= 0) & (freqs <= 8)]
    ...
    # Calculate the mean magnitudes in each frequency range
    # freqs = rfftfreq(len(ecg_signal))
    # fft_values = np.abs(rfft(ecg_signal))
    
    # detection_results['normal'] = np.mean(fft_values[(freqs >= 0) & (freqs <= 50)])
    # detection_results['atrial_fibrillation'] = np.mean(fft_values[(freqs > 50) & (freqs <= 100)])
    # detection_results['myocardial_infarction'] = np.mean(fft_values[(freqs >= 0) & (freqs <= 4)])
    # detection_results['sinus_rhythm'] = np.mean(fft_values[(freqs >= 0) & (freqs <= 8)])
...
def apply_slider_changes(ecg_signal, slider_values):
    ...
    fft_values[(freqs > 50) & (freqs <= 100)] *= slider_values['atrial_fibrillation']
    fft_values[(freqs >= 0) & (freqs <= 4)] *= slider_values['myocardial_infarction']
    fft_values[(freqs >= 0) & (freqs <= 8)] *= slider_values['sinus_rhythm']
